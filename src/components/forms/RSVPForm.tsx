...
import { Loader2, Users, Baby, ChefHat, AlertTriangle } from "lucide-react";
...
export function RSVPForm() {
  ...
  const [childrenUi, setChildrenUi] = useState<
  ...
  if (childrenUi.length !== childrenCount) {
    const next: { first: string; last: string; age: string }[] = Array.from(
  ...
  // (Removed global children validation effect; rely on inline messages + submit blocking)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const composedPlusOneName = plusOneAttending
      ? `${plusOneFirstName} ${plusOneLastName}`.trim().replace(/\s+/g, " ")
      : undefined;
    ...
    // Validate children details (only under 18). Do not surface a global inline error; block submit via toast.
    if (attending && childrenCount > 0) {
      const slice = childrenUi.slice(0, childrenCount);
      const invalidChildren = slice.some((c) => {
      ...
      if (invalidChildren) {
        ...
        toast.error("Моля, въведете валидни данни за децата (възраст 0–17)");
        return;
      }
    }
  ...
  return (
    <div className="grid gap-6 md:grid-cols-2">
              ...
              <Input
                ...
                className={`${
                  errors.guestFirstName || errors.guestName
                    ? "border-red-500"
                    : "border-gray-300"
                } bg-white text-black placeholder:text-gray-500 focus:border-primary focus:ring-2 focus:ring-primary/20`}
              />
              {errors.guestFirstName && (
                <p className="text-sm text-red-500">{errors.guestFirstName}</p>
              )}
            </div>
            <div className="space-y-2">
              <Label htmlFor="guestLastName" className="text-black font-medium">
                Фамилия *
              </Label>
              <Input
                id="guestLastName"
              ...
              {errors.guestLastName && (
                <p className="text-sm text-red-500">{errors.guestLastName}</p>
              )}
            </div>
          </div>
          {errors.guestName && (
            ...
            >
              <div className="flex items-center space-x-3 p-3 border border-gray-200 rounded bg-white hover:bg-gray-50">
                <RadioGroupItem
                  value="true"
                  id="attending-yes"
                  className="text-primary border-gray-300 focus:ring-2 focus:ring-primary/20"
                />
                <Label
        ...
        <CardContent className="space-y-6">
          {attending ? (
            <>
              <div className="flex items-center justify-between rounded-lg border border-gray-200 p-4 bg-white">
                <div className="space-y-0.5">
                  <Label className="text-base flex items-center gap-2 text-black">
                    <Users className="w-4 h-4" /> Ще доведете някого със себе
                    си?
                  </Label>
                  <p className="text-sm text-gray-600">
                    Партньор или приятел/ка
                  </p>
                </div>
                <Switch
              ...
              <div className="space-y-2">
                <Label className="flex items-center gap-2 text-black font-medium">
                  <Baby className="w-4 h-4" /> Брой деца
                </Label>
                <Select
                  ...
                  <SelectTrigger
                    className={`${
                      errors.childrenCount
                        ? "border-red-500"
                        : "border-gray-300"
                    } bg-white text-black focus:border-primary focus:ring-2 focus:ring-primary/20`}
                  >
              ...
              {attending && childrenCount > 0 && (
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    {Array.from({ length: childrenCount }).map((_, index) => (
                      ...
                      >
                        ...
                        <div className="space-y-1">
                          {(() => {
                            const ageStr = childrenUi[index]?.age || "";
                            const ageNum = parseInt(ageStr);
                            const invalidAge =
                              ageStr !== "" &&
                              (isNaN(ageNum) || ageNum < 0 || ageNum > 17);
                            const inputClass = `${
                              invalidAge ? "border-red-500" : "border-gray-300"
                            } bg-white text-black placeholder:text-gray-500 border focus:border-primary focus:ring-2 focus:ring-primary/20`;
                            return (
                              <>
                                <Input
                                  type="number"
                                ...
                                {invalidAge && (
                                  <p className="text-xs text-red-500">
                                    Възрастта трябва да е между 0 и 17
                                  </p>
                                )}
                              </>
                            );
                  ...
                  {/* Global children error removed in favor of inline per-field messages */}
                </div>
              )}

              <div className="space-y-2">
                <Label className="flex items-center gap-2 text-black font-medium">
                  <ChefHat className="w-4 h-4" /> Вашето меню *
                </Label>
                <Select
                  value={menuChoice}
                ...
                >
                  <SelectTrigger
                    className={`${
                      errors.menuChoice ? "border-red-500" : "border-gray-300"
                    } bg-white text-black focus:border-primary focus:ring-2 focus:ring-primary/20`}
                  >
              ...
              {plusOneAttending && (
                <div className="space-y-2">
                  <Label className="flex items-center gap-2 text-black font-medium">
                    <ChefHat className="w-4 h-4" /> Избор на меню за гост *
                  </Label>
                  <Select
              ...
              <div className="space-y-2">
                <Label
                  ...
                  className="flex items-center gap-2 text-black font-medium"
                >
                  <AlertTriangle className="w-4 h-4" /> Алергии или специални
                  изисквания
                </Label>
                <Textarea
                  id="allergies"
            ...
            <p className="text-sm text-gray-600">
              Ако няма да присъствате, не е необходимо да попълвате допълнителни
              данни.
            </p>
          )}
        </CardContent>
      ...
      <div className="md:col-span-2 space-y-4">
        <Button type="submit" form="rsvp-form" className="hidden" />
        <Button
          onClick={(e) => {
            e.preventDefault();
            void handleSubmit(e);
